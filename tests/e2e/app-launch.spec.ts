...
test.describe('App launch', () => {
  ...
  test('app title is visible in the top bar', async ({ page }) => {
    await expect(page.getByText('SmootheVoice Watermark Remover')).toBeVisible();
  });

  test('names the window after the app', async ({ page, electronApp }) => {
    // The fixture pins English, so this is the English brand name.
    await expect(page).toHaveTitle('SmootheVoice Watermark Remover');
    ...
    expect(name).toBe('SmootheVoice Watermark Remover');
  });
